...
`);s.gallery.insertAdjacentHTML("beforeend",e),B.refresh()},m=()=>{s.loader.classList.remove("hidden")},h=()=>{s.loader.classList.add("hidden")},M=()=>{s.gallery.innerHTML=""},P=()=>{s.loadMoreBtn.classList.add("shown")},f=()=>{s.loadMoreBtn.classList.remove("shown")};let l=1,d;s.searchForm.addEventListener("submit",async r=>{if(r.preventDefault(),d=r.target.elements["search-text"].value.trim(),!d){c.error({message:"Please, fill the input",position:"topRight"});return}l=1,M(),f(),m();try{const{hits:e,totalHits:i}=await u(d,l);if(!(e!=null&&e.length)){c.error({message:"Sorry, there are no images matching your search query. Please try again!",position:"topRight"});return}y(l,i),r.target.reset(),p(e)}catch(e){c.error({message:`There is an error with searching images: ${e}`,position:"topRight"}),console.log(e)}h()});s.loadMoreBtn.addEventListener("click",async()=>{f(),m();try{const{hits:r,totalHits:e}=await u(d,l+1);p(r),l++,y(l,e),window.scrollBy({top:s.gallery.firstChild.getBoundingClientRect().height*2,behavior:"smooth"})}catch(r){c.error({message:`There is an error during loading more images: ${r}`,position:"topRight"}),console.log(r)}h()});function y(r,e){r>=Math.ceil(e/g)?c.info({position:"topRight",message:"We're sorry, but you've reached the end of search results."}):P()}
